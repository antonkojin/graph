...
È la rappresentazione più naturale per delle relazioni.
Un albero binario di ricerca %albero 2-3 ???
per trovare un utente dato il suo
identificativo. Strutture alternative per la ricerca potrebbero essere: tabella hash, efficiente ma inadatta per insiemi di dimensione variabile dato che si dovrebbe ridimensionare la tabella; albero 2-3, più efficiente nel caso peggiore, 
...
Per trovare i gruppi di amici tale che ogni amicizia è stata instaurata dopo un certo anno, modifico la procedurà profondità nel modo seguente.
\begin{lstlisting}[mathescape]
profondità($v$)
  list_append( componente, $v$, anno)
  $M(v)$ := true
  for each $w \in L(v)$
